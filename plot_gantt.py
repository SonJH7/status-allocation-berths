...
class _DragManager:
    """matplotlib Rectangle 패치를 드래그할 수 있게 연결하는 헬퍼."""

    def __init__(self, entries: List[tuple[Rectangle, Optional[Text]]]):
        self._entries = entries
        self._active: Optional[tuple[Rectangle, Optional[Text], float, float, float, float]] = None

    def enable(self, canvas) -> None:
    ...
    def _on_press(self, event) -> None:
        if event.inaxes is None or event.xdata is None or event.ydata is None:
            return
        for rect, text in self._entries:
    ...
    def _on_motion(self, event) -> None:
        if self._active is None or event.xdata is None or event.ydata is None:
            return
        rect, text, press_x, press_y, rect_x, rect_y = self._active
        dx = event.xdata - press_x
        dy = event.ydata - press_y
        rect.set_x(rect_x + dx)
        rect.set_y(rect_y + dy)
        if text is not None:
            text.set_position((rect.get_x() + rect.get_width() / 2, rect.get_y() + rect.get_height() / 2))
...
def plot_berth_rectangles(
    ...
    *,
    ...
    enable_drag : bool, default False
        matplotlib 인터랙티브 백엔드 사용 시 드래그 앤 드롭 지원을 활성화할지 여부.
    """
    ...
    # 사각형 그리기
    rect_entries: List[tuple[Rectangle, Optional[Text]]] = []
    ...
    for _, row in normalized.iterrows():
        ...
        text_artist: Optional[Text] = None
        if show_labels:
        ...
        rect_entries.append((rect, text_artist))
    ...
    if enable_drag and rect_entries:
        manager = _DragManager(rect_entries)
        manager.enable(fig.canvas)
